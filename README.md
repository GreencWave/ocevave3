...
6. **관리자 시스템** ⭐ 완전 구현
   - 관리자 전용 페이지
   - 하드코딩된 관리자 계정
   - **상품 관리**: 추가, 수정, 삭제 (상품명, 설명, 가격, 재고, 이미지 URL, 카테고리)
   - **이벤트 관리**: 추가, 삭제 (제목, 내용, 일정, 장소, 이미지 URL)
   - **활동 관리**: 추가, 삭제 (제목, 내용, 날짜, 장소, 이미지 URL)
   - **주문 관리**: 주문 목록 조회 (주문번호, 고객 정보, 배송지, 금액, 상태)
...
1. **상품 관리**
   - 상품 추가: 상품명, 설명, 가격, 재고, 이미지 URL, 카테고리 입력
   - 상품 수정: 기존 상품 정보 변경
   - 상품 삭제: 상품 삭제 (확인 후 삭제)

2. **이벤트 관리**
   - 이벤트 추가: 제목, 내용, 일정, 장소, 이미지 URL 입력
   - 이벤트 삭제: 이벤트 삭제 (확인 후 삭제)

3. **활동 관리**
   - 활동 추가: 제목, 내용, 날짜, 장소, 이미지 URL 입력
   - 활동 삭제: 활동 삭제 (확인 후 삭제)
